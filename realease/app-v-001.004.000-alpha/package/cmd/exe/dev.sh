...
TARGET="$(jq -r '.target' /tmp/share/data/$SESSION/data.json)"
_PATH="/mnt/$TARGET/$(jq -r '.path' /tmp/share/data/$SESSION/data.json)"
...
if [[ $TARGET == "obsidian" ]]; then
    ...
    cp -rf /mnt/vault/.obsidian/snippets /mnt/export/data-$TARGET/snippets
...
diff -u <(echo "$F_DIR") <(echo "$DIR") | grep '^[+-]'
echo ""
F_DIR="$( echo "$F_DIR\n$DIR" | sort -u)"
...
while IFS= read -r I; do
    ...
    LINK_TARGET=${LINK_TARGET#"$STORAGE"}
